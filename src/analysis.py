...
EXCEL_FILE = (current_dir / '..' / 'data' / 'raw' / 'movies.xlsx').resolve()
CSV_OUTPUT_FILE = (current_dir / '..' / 'data' / 'processed' / 'movies_analysis.csv').resolve()
...
def get_searches(pytrends: TrendReq, title: str, timeframe: str, attempts: int = 5) -> float:
    ...
    Pytrends only gives relative search interest to itself, there normalisation
    is done by comparing the movie's search interest to the 'Feature film' anchor.
...
def main():
    ...
    if processed_movies:
        result_df = pd.DataFrame(processed_movies)
        result_df.to_csv(CSV_OUTPUT_FILE, index=False, encoding="utf-8")
        logging.info(f"CSV saved to {CSV_OUTPUT_FILE}.")
    else:
